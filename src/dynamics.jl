...
function lower_codomain(block::T) where {T <: Function}
    codom = type_dict(block)["codomain"]
    if is_shallow(codom)
        return Dict(name(codom) => codom)
    else
        return Dict(name(spc) => spc for spc in values(codom))
    end
end
...
function type_dict(block::T) where {T <: Function}
    ...
    result = Dict(domain_name[1] => type_obj
    for (domain_name, type_obj) in zip(str_types, svec_types))
